...
def search_dem_product(token, bbox):
    ...
    params = {"$filter": filter_str, "$top": 50}
...
def extract_dem(zip_file, out_dir):
    ...
    candidate_files = [f for f in all_files if f.suffix in extensions and "CoV" not in f.name]
    
    if not candidate_files:
        large_files = [f for f in all_files if f.is_file() and f.stat().st_size > 5000000]
        if large_files:
             dem_file = max(large_files, key=lambda f: f.stat().st_size)
             print(f"Guessing largest file is DEM: {dem_file.name}")
             return dem_file
...
def convert_to_geotiff(src_path, dst_path):
    ...
    with rasterio.open(src_path) as src:
        ...
        if nodata_val is None:
            if np.nanmin(data) < -32000: nodata_val = -32767
...
def _read_raster_with_profile(path: str) -> Tuple[Optional[np.ndarray], Optional[Dict]]:
    """Reads a raster and returns both the array and its profile (metadata)."""
    try:
        ...
        # print(f"Debug: Could not read {path}: {e}")
        return None, None
...
def topographic_module(
    ...
    for key, fname in DEFAULT_FILES.items():
        ...
        arr, profile = _read_raster_with_profile(path)
        data[key] = arr
        
        # Capture the profile from the filled DEM for integration
        if key == "dem_filled" and profile is not None:
            master_profile = profile
    
    # Store metadata for integration module!
    data["profile"] = master_profile

    if visualize:
        try:
            # Lazy import to avoid circular dependencies if any
            from fp.modules.data_visualization.data_visualization_module import visualize_topography_data_separate
            visualize_topography_data_separate(data, out_dir=out_dir)
        except ImportError:
            pass
